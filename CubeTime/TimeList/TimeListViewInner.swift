...
//        self.collectionView.register(UICollectionViewCell.self, forCellWithReuseIdentifier: timeResuseIdentifier)
    ...
    override func collectionView(_ collectionView: UICollectionView, contextMenuConfigurationForItemAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        ...
        let pen = PenTypes(rawValue: solve.penalty)!
        ...
        let sessions = (stopwatchManager.currentSession.session_type == SessionTypes.playground.rawValue ?
        ...
        let pinnedMenuItems = pinned.map { session in
            UIAction(title: session.name!, image: UIImage(systemName: iconNamesForType[SessionTypes(rawValue:session.session_type)!]!)) {_ in
